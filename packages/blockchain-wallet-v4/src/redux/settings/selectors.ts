...
export const getCountryCode = state =>
  getSettings(state).map(prop('country_code'))
...
export const getSmsVerified = state =>
  getSettings(state).map(prop('sms_verified'))
export const getEmail = state => getSettings(state).map(prop('email'))
export const getEmailVerified = state =>
  getSettings(state).map(prop('email_verified'))
export const getEmailVerifiedFailed = state =>
  getSettings(state).map(prop('email_verified_failed'))
export const getAutoLogout = state =>
  getSettings(state).map(prop('auto_logout'))
export const getLoggingLevel = state =>
  getSettings(state).map(prop('logging_level'))
...
export const getBlockTorIps = state =>
  getSettings(state).map(prop('block_tor_ips'))
...
export const getAuthTypeNeverSave = state =>
  getSettings(state).map(prop('never_save_auth_type'))
export const getGoogleAuthSecretUrl = state =>
  getSettings(state).map(prop('google_authenticator_secret_url'))
export const getNotificationsOn = state =>
...
export const getInvitations = (
  state
): RemoteDataType<string, InvitationsType> =>
